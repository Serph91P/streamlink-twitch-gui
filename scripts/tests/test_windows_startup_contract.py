...
class WindowsStartupContractTests(unittest.TestCase):
    def test_ci_checks_release_gui_subsystem_and_visible_startup_error(self) -> None:
        ...
        self.assertIn("--all-features --lib", workflow)
        self.assertIn("--all-features --test settings_store", workflow)
        self.assertIn("--all-features --test startup_failure", workflow)
        self.assertIn("--all-features --test streamlink_process", workflow)
